...
SMT solvers allow to analyse programs with various semantics or specifications but are limited for the manipulation of problems involving nonlinear arithmetic. Satisfiability (SAT)/SMT solvers can output proof witnesses which can be formally rechecked inside the $\coq$ proof assistant~\cite{smtcoq}. While ensuring soundness, the efficiency of the procedure is not compromised due to tactics enjoying the mechanism of computational reflection.
% Certified/Formal: SAT/SMT but not formalized yet (could be Rosa + SMTCoq)  + Gao
\cite{dReal13}
\paragraph{Abstract Domains}
% Interval domains Gappa (Flocq, coq-interval) 
% zonotopes octogons Fluctuat + Min\'e
% parametric polyhedra domains
\cite{Blanchet03,IntervalPoly09,Apron09}
