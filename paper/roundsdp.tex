...
\newcommand{\boundnlexprfun}[7]{\mathtt{bound\_nlexpr}(#1, #2, #3, #4, #5, #6, #7)}
\newcommand{\boundnlexpr}{\mathtt{bound\_nlexpr}}
...
Finally, we explain how to apply the \code{bound_nlexpr} procedure to nonlinear programs involving conditionals. 
%
\begin{figure}[!ht]
\begin{algorithmic}[1]
\Require variables $(\x,\e)$, constraints $\K = \X \times \E$, nonlinear expression \lstinline|$f = \ $ IfThenElse ($p, g, h$)|, rounded expression $\hat{f}$
\State Compute $I_d^p := \boundnlexprfun{\x}{\X}{p}{\hat{p}}{\e}{\E}{d} = [\underline{p_d}, \overline{p_d}]$
\State Define $\X_1 := \{ \x \in \X : 0 \leq p(\x) \leq \underline{p_d} \}$
\State Define $\X_2 := \{ \x \in \X : 0 \leq p(\x) \leq \underline{p_d} \}$
\Ensure upper bound $f_d$ of the error $\mid \hat{f} - f  \mid$ over $\K$
...
\State \Return $\max \{f_d^1, f_d^2, f_d^3, f_d^4 \}$
\end{algorithmic}
\caption{\code{bound_cndexpr}}
\label{alg:bound_cndexpr}
\end{figure}
%
