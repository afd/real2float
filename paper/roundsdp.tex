...
\newcommand{\sdptrancs}{\mathtt{sdp\_transc}}
...
The algorithm \code{bound_nlexpr}, depicted in Figure~\ref{alg:bound_nlexpr}, takes as input $\x$, $\X$, $f$, $\hat{f}$, $\e$ as well as the set $\E$ of bound constraints over $\e$. For a given machine $\epsilon$, one has $\E := [-\epsilon, \epsilon]^m$, with $m$ being the number of error variables. This algorithm actually relies on the sparse SDP optimization procedure $(\S_d)$ (see Section~\ref{sec:sdpbackground} for more details), thus~\code{bound_nlexpr} also takes as input a relaxation order $d \in \N$. The algorithm provides as output an upper bound of the maximal absolute error bound $r^\star := \max_{(\x,\e)\in \K} \mid \hat{f}(\x,\e) - f(\x) \mid $.

\begin{figure}[!ht]
\begin{algorithmic}[1]                    
\Require input variables $\x$, input constraints $\X$, nonlinear expression $f(\x)$, rounded expression $\hat{f}(\x, \e)$, error variables $\e$, error constraints $\E$, relaxation order $d$
...
\paragraph{Non-polynomial programs.} We first illustrate the extension to semialgebraic programs with the following example.
\begin{example}
...
Let $\K := [0, 1] \times [-\epsilon, \epsilon]^2$. To use the sparse SDP program~\eqref{eq:lscalesdp2}, the idea is to transform the rational optimization problem $\inf_{(x_1,\e) \in \K} l (x_1, \e)$ into a polynomial optimization problem. One way to do it is to introduce a lifting variable $x_2 := \frac{x_1}{1 + x_1}$ to handle the division operator and to consider the equality constraint $p(x) := x_1 - x_2 (1 + x_1) = 0$, encoded by the two inequality constraints $p (\x) \geq 0$ and $-p(\x) \geq 0$. To ensure the compactness assumption, one can bound $x_2$ with the enclosure $[0, 1/2]$, using basic interval arithmetic.

Let $\Kpol := \{(\x,\e) \in [0, 1] \times [0, 1/2] \times [\epsilon, \epsilon]^2 : p(\x) \geq 0 \,,\  - p(\x) \geq 0 \}$. Then the rational optimization problem involving $l$ is equivalent to $\inf_{(\x,\e) \in \Kpol} x_2 (-e_1 + e_2)$, a polynomial optimization that we can handle with the $\sdpbound$ procedure, described in Section~\ref{sec:polsdp}.
\end{example}
%
