...
\multirow{1}{*}{floudas2}
& (double) &  $9.03\text{e--}16$ & $1.12\text{e--}15$ & $9.96\text{e--}16$ & $1.12\text{e--}15$ & ? \\
\multirow{1}{*}{floudas3}
...
%\small
\begin{center}
\caption{Comparison of CPU timings (in seconds) for absolute rounding errors (the best results are emphasized using \textbf{bold fonts})}
...
%
We have presented a verification framework to over-approximate rounding errors occurring while executing nonlinear programs implemented with finite precision.
...
First, we intend to increase the size of graspable instances by taking into account symetry patterns related to certain program sub-classes. Next, we plan to improve the efficiency of the formal polynomial checker inside Coq by using the interval libraries available inside the proof assistant, rather than using exact polynomial arithmetic in the current framework. 
We also intend to provide rounding error bounds for more general programs, involving both finite and infinite loops,  . Such extensions has been made realistic that recent efforts to approximate inductive invariants with semidefinite relaxations
%In the sequel, we briefly outline some of these tracks.

1) lower bounds with other hierarchies
2) fixed precision
3) while loops
5) Formalization of maxplus (cf JFR)
