...
We adopt the standard practice~\cite{higham2002accuracy} to approximate a real number $x$ with its closest floating-point representation $\hat{x} = x (1 + e)$, with $|e|$ being less than the machine precision $\epsilon$. This model is only valid when neglecting both overflow and denormal range values.
...
%Certified/Formal: SAT/SMT but not formalized yet (could be!) + Gappa (Flocq, coq-interval) + FPTaylor
