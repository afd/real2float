...
All examples are displayed in Appendix A as the corresponding $\realtofloat$ input text files and satisfy our nonlinear program semantics (see Section~\ref{sec:fpbackground}).
For each example, we compared the quality of the rounding error bounds and corresponding execution times while running our tool $\realtofloat$, $\fptaylor$ (version from May $2015$)~\cite{fptaylor15} and $\rosa$ (version from June $2015$)~\cite{Darulova14Popl}. All benchmark results have been obtained on an Intel Core i5 CPU ($2.40\, $GHz).

For each problem presented in Table~\ref{table:error}, we certify an upper bound of the rounding error.

\if{
 function $f$ on a box $K$. The semialgebraic optimization problems are solved at the SOS relaxation order $k$. When the relaxation gap is too high to certify the requested
bound, then we perform a domain subdivision in order to get tighter bounds:
we divide the maximal width interval of $K$ in two halves to get two sub-boxes $K_1$ and $K_2$ such that $K = K_1 \cup K_2$. We repeat this subdivision procedure,  by applying $\templateoptim$ on a finite set of sub-boxes,
until we succeed to certify that $m$ is a lower bound of $f$.
We note $\nbb$ the total number of sub-boxes generated
by the algorithm.
}\fi
...
& (double) & $2.75\text{e--}13$ & $4.97\text{e--}13$ & $\mathbf{1.57\textbf{e--}13}$ & $\nan$ & $7.11\text{e--}14$\\
\multirow{1}{*}{doppler2} & \texttt{b}
& (double) & $8.04\text{e--}13$ & $1.29\text{e--}12$ & $\mathbf{2.87\textbf{e--}13}$ & $\nan$ & $1.14\text{e--}13$\\
\multirow{1}{*}{doppler3} & \texttt{c}
& (double) & $1.37\text{e--}13$ & $2.03\text{e--}13$ & $\mathbf{8.16\textbf{e--}14}$ & $\nan$ & $4.27\text{e--}14$\\
