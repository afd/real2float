...
First, note that one can map each input variables $x_i$ to the integer $i$, for all $i=1,\dots,n$, as well as each error variable $e_i$ to $n+i$, for all $i=1,\dots,m$. Then, define the sets $C_1 := {1,\dots,n,n+1}, \dots, C_m := {1,\dots,n,n+m}$. Here, we take advantage of the sparsity correlation pattern of $l$ by using $m$ distinct sets of cardinal $n+1$ rather than a single one of cardinal $n+m$, i.e.~the total number of variables. 
We also scale the optimization problems by writing 
\begin{align}
\label{eq:lscale}
l(\x,\e) = \sum_{i=1}^m s_i (\x) e_i = \epsilon \sum_{i=1}^m s_i (\x) \frac{e_i}{\epsilon} \,,
\end{align}
%
leading to compute an interval enclosure of $l/\epsilon$ over $\K' := \X \times [-1, 1]^m$.
...
\end{align*}
%
with $g_{k+i}(\x, e_i) := M + 1 -  \sum_{i=1}^n x_i^2 + e_i^2$, for all $i=1,\dots, m$.
Then, one can compute the minimum of $l(\x,\e)$ over $\K$ by solving the following optimization problem:
\[
\left\{			
\begin{array}{rl}
\underline{l_d} := \sup\limits_{\mu, \sigma_j} & \mu\enspace, \\	 
\text{s.t.} & l (\x, \e) - \mu = \sum\limits_{j = 0}^{k'} \sigma_j(\x) g_j(\x) \,, \ \forall \x \in \R^n \,, \\
& \mu\in \R \,, \ \sigma_j \in \Sigma[\x, F_j] \,, \\
& \sigma_0 \in \sum\limits_{j = 1}^m \Sigma [\x, C_m] \,, \\
& \deg (\sigma_j g_j) \leq 2 d  \,, \ j = 1,\dots,k' \,.
\end{array} \right.
